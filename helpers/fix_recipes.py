...
with open('data/recipes.15.31_dumb.yaml') as yf:
    ...
    for (recipe_name, recipe) in recipes.items():
        ...
        if 'category' in recipe:
            if recipe['category'] == 'chemistry' and 'results' in recipe:
                clean_recipe['result_count'] = recipe['results'][0]['amount']
            elif recipe['category'] == 'oil-processing':
                clean_recipe['results'] = {res['name']: res['amount'] for res in recipe['results']}
